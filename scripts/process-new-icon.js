...
async function processNewIcon() {
  const newImagePath = "C:/Users/Kuppan C T/.gemini/antigravity/brain/b781f5a9-b6dd-42af-8911-196ef93cc9ee/.user_uploaded/media_1787944622742.jpg";

  if (!fs.existsSync(newImagePath)) {
    console.error("Image file not found:", newImagePath);
    return;
  }

  console.log("Processing uploaded icon:", newImagePath);
  ...
  console.log(`Average Corner Background RGB: (${bgR.toFixed(1)}, ${bgG.toFixed(1)}, ${bgB.toFixed(1)})`);
  ...
  const isBgMatch = (r, g, b) => {
    ...
    return dist < 35; // Tolerance for outer background
  };
    ...
    for (let x = 1; x < width - 1; x++) {
      ...
      if (data[idx + 3] > 0) {
        ...
        if (transNeighbors > 0) {
          ...
          if (dist < 45) {
            data[idx + 3] = Math.round(255 * (dist / 45));
          }
        }
      }
    }
  ...
  console.log("✅ Successfully saved new transparent 3D icon to public/icon.png, app/icon.png, and favicons!");
}

processNewIcon().catch(console.error);
